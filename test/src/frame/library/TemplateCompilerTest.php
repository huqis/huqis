...
class TemplateCompilerTest extends PHPUnit_Framework_TestCase {
    ...
    public function providerCompile() {
        return array(
            ...
            // macro block
            array('{macro myFirstMacro()}loop{/macro}', '$macro1 = function(TemplateContext $context) { echo "loop"; };$context->setFunction(\'myFirstMacro\', new \frame\library\func\MacroTemplateFunction("myFirstMacro", $macro1));'),
            array('{macro myMacro($input, $input2)}loop {$input}{/macro}', '$macro1 = function(TemplateContext $context) { echo "loop ";echo $context->getVariable(\'input\', false); };$context->setFunction(\'myMacro\', new \frame\library\func\MacroTemplateFunction("myMacro", $macro1, [\'input\', \'input2\'], []));'),
            array('{macro sum($input, $input2 = 2)}{return $input + $input2}{/macro}', '$macro1 = function(TemplateContext $context) { return $context->getVariable(\'input\', false) + $context->getVariable(\'input2\', false); };$context->setFunction(\'sum\', new \frame\library\func\MacroTemplateFunction("sum", $macro1, [\'input\', \'input2\'], [1 => 2]));'),
            // extendable block
