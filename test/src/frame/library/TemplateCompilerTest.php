...
class TemplateCompilerTest extends PHPUnit_Framework_TestCase {
    ...
    public function providerCompile() {
        return array(
            ...
            // macro block
            array('{macro myFirstMacro()}loop{/macro}', '$macro1 = function(TemplateContext $context) {  ?>loop<?php  };$macro1 = new MacroTemplateFunction($macro1);$context->setFunction(\'myFirstMacro\', $macro1);'),
            array('{macro myMacro($input, $input2)}loop {$input}{/macro}', '$macro1 = function(TemplateContext $context) {  ?>loop <?php echo $context->getVariable(\'input\'); };$macro1 = new MacroTemplateFunction($macro1, [\'input\', \'input2\']);$context->setFunction(\'myMacro\', $macro1);'),
            array('{macro sum($input, $input2)}{return $input + $input2}{/macro}', '$macro1 = function(TemplateContext $context) { return $context->getVariable(\'input\') + $context->getVariable(\'input2\'); };$macro1 = new MacroTemplateFunction($macro1, [\'input\', \'input2\']);$context->setFunction(\'sum\', $macro1);'),
            // extendable block
